/*
 ...
 AI self-playing added by Luke Ceddia for a COMP20003 project
 ============================================================================
 */
...
void dump_stats(stats_t *statistics, uint8_t board[SIZE][SIZE]) {
    ...
    for (i = 0; i < SIZE; i++) {
      for (j = 0; j < SIZE; j++) {
        max_tile = board[i][j] > max_tile ? board[i][j] : max_tile;
      }
    }
    ...
    fprintf(fhandle,
           ...
           statistics->expanded / statistics->total_time,
           1 << max_tile,
           statistics->score);
    fclose(fhandle);
}
...
int main(int argc, char *argv[]) {
	...
	/**
	 *  register signal handler for fatal signals (so we can leave
	...
	while (true) {
		 ...
		 */
		move_t selected_move = get_next_move( board, max_depth, propagation, &statistics );
